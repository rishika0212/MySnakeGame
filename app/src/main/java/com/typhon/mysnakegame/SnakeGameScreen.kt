...
import com.typhon.mysnakegame.ui.theme.Citrine
import com.typhon.mysnakegame.ui.theme.Custard
import com.typhon.mysnakegame.ui.theme.Red
import com.typhon.mysnakegame.ui.theme.RoyalBlue
...
fun SnakeGameScreen(
    ...
    val foodImageBitmap= ImageBitmap.imageResource(id = R.drawable.mouse2)
    val snakeHeadImageBitmap: ImageBitmap = when(state.direction){
        ...
        Column(
            ...
            Canvas(
                modifier = Modifier
                    ...
                    .pointerInput(state.gameState){
                        if(state.gameState!=GameStat.STARTED){
                            return@pointerInput
                        }
            ...
            Row(
                ...
                Button(
                    ...
                    enabled=state.gameState==GameStat.PAUSED||state.isGameOver
                ){
                ...
                Button(
                    ...
                    onClick ={
                        when(state.gameState){
                            GameStat.IDLE, GameStat.PAUSED -> onEvent(SnakeGameEvent.StartGame)
                            GameStat.STARTED -> onEvent(SnakeGameEvent.PauseGame)
                        }
                    },
                    ...
                    Text(
                        text = when(state.gameState){
                            GameStat.IDLE -> "Start"
                            GameStat.STARTED -> "Pause"
                            GameStat.PAUSED -> "Resume"
                        }
                    )
                }
